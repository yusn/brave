...
function process_simple_like($request_data) {
	try {
		// Security
		$nonce = isset( $request_data['nonce'] ) ? sanitize_text_field( $request_data['nonce'] ) : 0;
		// $nonce = wp_create_nonce( 'simple-likes-nonce' ); // Security
		if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
			return new WP_Error( 404, __( 'Not permitted', 'brave' ) ); // new WP_Error($error_code, $message);
		}
		// Test if javascript is disabled
		$disabled = ( isset( $request_data['disabled'] ) && $request_data['disabled'] == true ) ? true : false;
		// Test if this is a comment
		$is_comment = ( isset( $request_data['is_comment'] ) && $request_data['is_comment'] == 1 ) ? 1 : 0;
		// Base variables
		$post_id = ( isset( $request_data['post_id'] ) && is_numeric( $request_data['post_id'] ) ) ? $request_data['post_id'] : '';
		$result = array();
		$post_users = NULL;
		$like_count = 0;
		// Get plugin options
		if ( $post_id != '' ) {
			...
			// Clear post cache of WP-Super-Cache for anonymous user
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	$output = '<span class="sl-button cur' . $post_id_class . $class . $comment_class . '" data-nonce="' . $nonce . '" data-post-id="' . $post_id . '" data-iscomment="' . $is_comment . '" title="' . $title . '">' . $icon . $count . '</span>' . $loader;
	return $output;
...
function get_liked_icon() {
	...
	$icon = '<span class="i-heart touch-easy sl-icon"></span>';
	return $icon;
} // get_liked_icon()
...
function get_unliked_icon() {
	...
	$icon = '<span class="i-heart-o touch-easy sl-icon"></span>';
	return $icon;
} // get_unliked_icon()
...
function get_like_count( $like_count ) {
	...
	if ( is_numeric( $like_count ) && $like_count > 0 ) { 
		$number = '<span class="sl-count">' . sl_format_count( $like_count ) . '</span>';
	} else {
		$number = $like_text;
	}
	$count = $number;
...
function show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'brave' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php _e( 'You do not like anything yet.', 'brave' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
